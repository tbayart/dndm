...
namespace MyDownloader.App.UI
{
    public partial class DownloadList : UserControl, IClipboardDataHandler
    {
        ...
        {
            InitializeComponent();
            lvwDownloads.GetType().GetProperty("DoubleBuffered", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).SetValue(lvwDownloads, true, null);
            lvwSegments.GetType().GetProperty("DoubleBuffered", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance).SetValue(lvwSegments, true, null);
            ...
            if (modal)
            {
                using (NewDownloadForm newDownload = new NewDownloadForm())
                {
                    if (!String.IsNullOrEmpty(url))
                    {
                        ...
                        rl[0]= ResourceLocation.FromURL(url);
                        newDownload.DownloadLocation =rl;
                    }
        ...
        public void StartSelections()
        {
            DownloadsAction(
                ...
                {
                    d.ScheduleState=DownloaderScheduleState.Waiting;
                    DownloadManager.Instance.StartNextDownload();
                    //d.Start();
                }
            );
        }
                ...
                try
                {
                    ...
                    DownloadsAction(
                        delegate(Downloader d, ListViewItem item)
                        {
                            ...
                            System.IO.FileInfo fi = new FileInfo(d.LocalFile);
                            if (fi.Exists)
                            try
                            {
                                fi.Delete();
                            ...
                            {
                                MessageBox.Show("Unable to delete: " + fi.FullName);
                            }
                        }
                    );
                }
