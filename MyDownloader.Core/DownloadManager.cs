...
namespace MyDownloader.Core
{
    public class DownloadManager
    {
        ...
        private int maxSimultaneusDownloads = 10;
        ...
        public void Add(Downloader downloader, bool autoStart)
        {
            ...
            if (autoStart)
                downloader.ScheduleState = DownloaderScheduleState.Waiting;
            else
                downloader.ScheduleState = DownloaderScheduleState.Paused;
            //if (autoStart)
            //{
            //    downloader.Start();
            //}
        ...
        public void StartNextDownload()
        {
            ...
            foreach (Downloader d in downloads)
                if (d.ScheduleState == DownloaderScheduleState.Working)
                    cnt++;
            if(cnt>=maxSimultaneusDownloads)
                return;
            foreach (Downloader d in downloads)
            {
                if (d.ScheduleState == DownloaderScheduleState.Waiting)
                {
                    d.Start();
                    Thread.Sleep(500);
                    return;
