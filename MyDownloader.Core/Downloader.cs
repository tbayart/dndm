...
namespace MyDownloader.Core
{
    public class Downloader
    {
        ...
        public DownloaderScheduleState ScheduleState;
        ...
        {
            get { return localFile.Substring(localFile.LastIndexOf("\\")+1); }
        }
        ...
        {
            if(this.LocalFile.EndsWith("/"))
                this.localFile = this.localFile.Substring(0, this.localFile.Length - 1) + this.remoteFileInfo.FileName;
            else
                if(this.LocalFile.EndsWith("\\"))
                    this.localFile += this.remoteFileInfo.FileName;
